...
class ParonCLI {
      ...
      while (!target) {
        target = (
          await this._question(
            'Anna luotavan huoneen nimi (esim. "palaveri"): ',
          )
        ).trim();
      }
    } else {
      while (!target) {
        target = (await this._question("Anna huoneen koodi tai nimi: ")).trim();
      }
    }
  ...
  async _enterRoom(roomInput) {
    ...
    if (this.audioEnabled) {
      ...
      if (audioStarted) {
        console.log(
          "[Audio] Mikrofoni ja kaiuttimet kytketty (48kHz Opus HD).",
        );
      } else {
        console.log(
          "[Audio] Äänilaitetta ei voitu käynnistää, jatketaan tekstichat-tilassa.",
        );
      }
    }
    ...
    try {
      ...
      console.error(
        "\n[Virhe] Huoneeseen liittyminen epäonnistui:",
        err.message,
      );
    ...
    console.log(
      "   /vad <0-500>    - Aseta mikrofonin VAD-herkkyys (oletus 150)",
    );
    ...
    console.log("   (Kirjoita tekstiä ja paina Enter lähettääksesi viestin)");
    console.log("-".repeat(60));
    console.log(
      `\nOlet huoneessa nimellä "${this.name}". Mikrofoni: ${this.room.isMuted ? "MYKISTETTY" : "PÄÄLLÄ (48kHz Opus HD)"}.\n`,
    );
  ...
  _bindRoomEvents() {
    this.room.on("peer-join", (peer) => {
      if (this.audio) {
        this.audio.addPeer(peer.peerId, { name: peer.name });
      }
      this._printLine(
  ...
  _startCommandLoop() {
    ...
    this.rl.on("line", (line) => {
      ...
      if (input.startsWith("/vad")) {
        if (!this.audio) {
          console.log("\nÄäni ei ole käytössä tässä istunnossa.\n");
          this.rl.prompt();
          return;
        }
        ...
        if (parts.length > 1 && !isNaN(parts[1])) {
          ...
          console.log(
            `\nMikrofonin VAD-herkkyys asetettu: ${newThreshold} (0 = aina päällä, 150 = oletus)\n`,
          );
        } else {
          console.log(
            `\nNykyinen VAD-herkkyyskynnys: ${this.audio.getVAD()} (Aseta uusi: /vad <0-500>)\n`,
          );
        }
        this.rl.prompt();
        return;
      ...
      if (input.startsWith("/")) {
        console.log(
          `Tuntematon komento: "${input}". Komennot: /mute, /vad, /users, /code, /devices, /quit`,
        );
        this.rl.prompt();
        return;
      }
