...
class ParonAudio extends EventEmitter {
  constructor(options = {}) {
    ...
    // Decoders for remote peers: peerId -> { decoder, name }
    this.peers = new Map();
  }
  ...
  addPeer(peerId, { name } = {}) {
    ...
    this.peers.set(peerId, { decoder, name: name || "Tuntematon" });
  }
  ...
  start() {
    if (!portAudio) {
      this.emit(
        "warn",
        "Natiivia äänirajapintaa (naudiodon) ei ole saatavilla.",
      );
      return false;
    }
    ...
    try {
      ...
      this.ai.on("error", (err) => {
        this.emit("error", new Error(`Mikrofonivirhe: ${err.message}`));
      });
      ...
      this.ao.on("error", (err) => {
        this.emit("error", new Error(`Kaiutintoiston virhe: ${err.message}`));
      });
      ...
      this.emit(
        "error",
        new Error(`Äänilaitteiden alustus epäonnistui: ${err.message}`),
      );
      this.stop();
      return false;
    }
