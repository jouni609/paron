...
class ParonRoom extends EventEmitter {
  constructor(options = {}) {
    ...
    this.swarm = null;
    this.discovery = null;
    this.peers = new Map(); // peerId -> { peerId, name, isMuted, messages, conn }
    this.opened = false;
    this.closing = false;
  }
  ...
  static deriveTopic(input) {
    ...
    if (!clean) throw new Error("Room name or code cannot be empty.");
    ...
    if (/^[0-9a-fA-F]{64}$/.test(clean)) {
      return {
        ...
        roomName: "Code room (" + clean.slice(0, 8) + "...)",
      };
    }
    ...
    const topic = crypto
      .createHash("sha256")
      .update("paron:v1:room:" + normalized)
      .digest();
  ...
  async join(roomInput) {
    if (this.opened) throw new Error("Room is already open.");
  ...
  _handleConnection(conn) {
    ...
    const channel = mux.createChannel({
      ...
      onclose: () => {
        if (remotePeerId && this.peers.has(remotePeerId)) {
          ...
          // Only trigger leave and delete if this was the active connection
          if (peer.conn === conn) {
    ...
    const handshakeMsg = channel.addMessage({
      ...
      onmessage: (msg) => {
        ...
        remoteName = msg.name || "Unnamed";
        remoteIsMuted = !!msg.isMuted;

        // Clean up old duplicate connection if exists
