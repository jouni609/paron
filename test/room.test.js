...
test('Room topic derivation from name and 64-hex code', (t) => {
  ...
  t.not(byName1.roomCode, byNameDifferent.roomCode, 'Different names produce different codes')

  const byHex = ParonRoom.deriveTopic(byName1.roomCode)
  t.is(byHex.roomCode, byName1.roomCode, 'Joining via 64-hex code produces same topic')
})
